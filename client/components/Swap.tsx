...
import { buildFundContractTx, buildDepositTx, submitSorobanTx } from "../lib/stellar";
...
function addressToField(address: string): string {
  ...
  if (StrKey.isValidMed25519PublicKey(address)) {
    decoded = new Uint8Array(StrKey.decodeMed25519PublicKey(address).slice(0, 32));
  } else {
...
type TxStatus =
  ...
  | { stage: "proving" }
  | { stage: "relaying" }
  | { stage: "success"; fundHash: string; depositHash: string; withdrawHash: string; xlmAmount: string }
...
export default function Swap() {
  ...
  const isLoading = [
    "generating", "funding", "signing-fund", "submitting-fund",
    "building", "signing", "depositing",
    "computing", "proving", "relaying",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const depositXdr = await buildDepositTx(address, commitmentData.commitment);
      ...
      console.log("[Send] ZK proof generated, size:", proofResult.proof.length, "bytes");
      ...
      console.log("[Send] Withdrawal confirmed:", data.txHash);
  ...
  const buttonLabel = () => {
    switch (txStatus.stage) {
      ...
      case "funding":        return "Building transfer…";
      case "signing-fund":   return "Sign transfer in wallet…";
      case "submitting-fund":return "Transferring XLM…";
      case "building":       return "Building deposit…";
      case "signing":        return "Sign deposit in wallet…";
      case "depositing":     return "Storing commitment…";
      case "computing":      return "Computing Merkle root…";
      case "proving":        return "Generating ZK proof…";
      case "relaying":       return "Sending to recipient…";
      default: return (<>Send <IoIosSend /></>);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="flex flex-col">
          ...
          <div className="px-5 pt-5 pb-3">
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 flex items-center gap-2 bg-gray-50 rounded-xl px-4 py-3 border border-gray-100 focus-within:border-[#9EB8E9] focus-within:ring-2 focus-within:ring-[#9EB8E9]/30 transition-all">
                <label htmlFor="swap-recipient" className="text-sm text-gray-400 sora-font shrink-0">To</label>
                <input
                  id="swap-recipient"
                  type="text"
                  value={recipient}
          ...
          <div className="flex flex-col items-center py-6 px-5">
            <div className="relative w-full flex justify-center">
              <div className={`text-center ${amountSizeClass} font-light text-gray-800 w-full sora-font flex items-center justify-center min-w-0 overflow-hidden transition-all duration-200`}>
                ...
                />
                {!showUsd && (
                  <span className={`text-gray-400 ${suffixSizeClass} ml-2 shrink-0 transition-all duration-200`}>XLM</span>
                )}
              </div>
            </div>
            <button
          ...
          <div className="mx-5 mb-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0 bg-gray-50 rounded-2xl px-4 py-3 border border-gray-100">
            <div className="flex items-center gap-3">
              <div className="rounded-xl flex items-center justify-center overflow-hidden shadow-sm">
                <Image src="/stellar.svg" alt="Stellar" width={28} height={28} className="w-10 h-10" />
              </div>
              <div className="flex flex-col">
                <span className="text-sm font-semibold text-gray-800 sora-font">Stellar</span>
            ...
            <div className="flex items-center gap-2">
              <button type="button" onClick={handleUseHalf} className="px-3.5 py-1.5 rounded-full bg-white border border-gray-200 text-xs font-medium text-gray-600 hover:bg-gray-100 hover:border-gray-300 active:scale-95 transition-all sora-font cursor-pointer shadow-sm">1/2</button>
              <button type="button" onClick={handleUseMax} className="px-3.5 py-1.5 rounded-full bg-white border border-gray-200 text-xs font-medium text-gray-600 hover:bg-gray-100 hover:border-gray-300 active:scale-95 transition-all sora-font cursor-pointer shadow-sm">MAX</button>
              <button type="button" onClick={() => setAmount("")} className="px-3.5 py-1.5 rounded-full bg-white border border-gray-200 text-xs font-medium text-gray-600 hover:bg-red-50 hover:text-red-500 hover:border-red-200 active:scale-95 transition-all sora-font cursor-pointer shadow-sm">CLR</button>
            </div>
          </div>
            ...
            <div className="mx-5 mb-4">
              ...
              <div className="w-full bg-gray-100 rounded-full h-1.5 overflow-hidden">
                <div
                  ...
                  style={{
                    width:
                      txStatus.stage === "generating"      ? "5%" :
                      txStatus.stage === "funding"         ? "10%" :
                      txStatus.stage === "signing-fund"    ? "15%" :
                      txStatus.stage === "submitting-fund" ? "25%" :
                      txStatus.stage === "building"        ? "30%" :
                      txStatus.stage === "signing"         ? "35%" :
                      txStatus.stage === "depositing"      ? "45%" :
                      txStatus.stage === "computing"       ? "55%" :
                      txStatus.stage === "proving"         ? "75%" :
                      txStatus.stage === "relaying"        ? "90%" : "0%",
                  }}
                />
              </div>
            </div>
          ...
          {txStatus.stage === "success" && (
            <div className="mx-5 mb-4 px-4 py-3 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm sora-font animate-[fadeIn_0.3s_ease-out] relative">
              ...
              <p className="font-semibold text-center">{txStatus.xlmAmount} XLM sent privately</p>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-gray-800 sora-font">Scan QR Code</span>
              </div>
              <button type="button" onClick={stopScanner} className="text-gray-400 hover:text-gray-600 transition-colors cursor-pointer text-xl leading-none p-1">✕</button>
            </div>
            <div className="p-4">
              <div id={scannerContainerId} className="w-full rounded-2xl overflow-hidden" style={{ minHeight: 280 }} />
            </div>
            <div className="px-5 pb-4 text-center">
              <p className="text-xs text-gray-400 sora-font">Point your camera at a Stellar address QR code</p>
            </div>
          </div>
        </div>
      )}
