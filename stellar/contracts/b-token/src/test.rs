...
fn create_client<'a>(e: &Env, owner: &Address, initial_supply: i128) -> BTokenClient<'a> {
    ...
    let symbol = String::from_str(e, "BT");
    let address = e.register(BToken, (name, symbol, owner, initial_supply));

    BTokenClient::new(e, &address)
}
...
fn initial_state() {
    ...
    assert_eq!(client.symbol(), String::from_str(&e, "BT"));
    assert_eq!(client.name(), String::from_str(&e, "Before Token"));
